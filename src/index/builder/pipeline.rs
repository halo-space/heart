...
impl Builder {
    ...
    pub async fn build(&self, items: Vec<source::Item>) -> Result<Vec<pagewiki::PageWiki>, Error> {
        ...
        for item in items {
            ...
            // 步骤 3：切分
            let mut drafts = cut_impl.cut(&normalized).await.map_err(|e| {
                tracing::warn!(stage = "cut", error = %e, "index.build.failed");
                e
            })?;
            ...
            for (idx, mut pw) in drafts.into_iter().enumerate() {
                ...
                assemble_system_fields(&mut pw, &item, idx, &version).map_err(|e| {
                    tracing::warn!(stage = "assemble", error = %e, "index.build.failed");
                    e
                })?;
                ...
                pw.metadata.remove("doc_id");
                promote::apply_promote(&mut pw, &self.metadata_promote_fields).map_err(|e| {
                    tracing::warn!(stage = "promote", error = %e, "index.build.failed");
                    e
                })?;

                // 步骤 7a：分词（content / keywords / questions 三路）
                pw.content_tokens = self.tokenizer.tokenize(&pw.content).await.map_err(|e| {
                    tracing::warn!(stage = "tokenize", error = %e, "index.build.failed");
                    Error::Tokenize(e.to_string())
                })?;
                pw.keyword_tokens = self
                    .tokenizer
                    .tokenize(&pw.keywords.join(" "))
                    .await
                    .map_err(|e| {
                        tracing::warn!(stage = "tokenize", error = %e, "index.build.failed");
                        Error::Tokenize(e.to_string())
                    })?;
                pw.question_tokens = self
                    .tokenizer
                    .tokenize(&pw.questions.join(" "))
                    .await
                    .map_err(|e| {
                        tracing::warn!(stage = "tokenize", error = %e, "index.build.failed");
                        Error::Tokenize(e.to_string())
                ...
                if let Some(embedder) = &self.embedder {
                    pw.embedding = Some(embedder.embed(&pw.content).await.map_err(|e| {
                        tracing::warn!(stage = "embed", error = %e, "index.build.failed");
...
mod tests {
    ...
    async fn 缺少scenario返回错误() {
        ...
        let item = source::Item {
            text: "x".into(),
            scenario: source::Scenario::Manual,
            metadata: meta,
        };
    ...
    async fn 同批次version一致() {
        ...
        let versions: Vec<_> = pages
            .iter()
            .map(|p| p.version.as_deref().unwrap())
            .collect();
        assert!(
